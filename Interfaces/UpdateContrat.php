<?php
    ...
    if(isset($_POST["id"]) && !empty($_POST["id"])){
        ...
        //  } elseif(!filter_var($input_num, FILTER_VALIDATE_REGEXP, 
        //      array("options"=>array("regexp"=>"/^[\p{Arabic}a-zA-Z_\p{N}]+\h?[\p{N}\p{Arabic}a-zA-Z_]*$/u")))){
        //      $NUM_CT_err = "لا يمكن أن يحتوي رقم العقد إلا على أحرف وأرقام وشرطات سفلية ";
         } else{
            $NUM_CT = $input_num; 
         }
        ...
        if(empty($NUM_CT_err) && empty($DT_CT_err) && empty($OBJ_CT_err) && empty($ENTITE_err) && empty($MNT_BRUT_err) && empty($MNT_BRUT_LTR_err) && empty($ID_TYPE_CT_err) && empty($ID_CTR_err)
        && empty($MNT_ARN_err) && empty($MNT_ARN_LTR_err) && empty($MNT_NET_err) && empty($MNT_NET_LTR_err)){

        // Prepare an update statement
        $query = "UPDATE contrat SET NUM_CT = ?, DT_CT = ?, OBJ_CT = ?, ENTITE = ?, ID_CTR = ?, ID_TYPE_CT = ?, PRM_CT = ?, AR1_CT = ?, 
        AR2_CT = ?, MNT_BRUT = ?, MNT_BRUT_LTR = ?, MNT_ARN = ?, MNT_ARN_LTR = ?, MNT_NET = ?, MNT_NET_LTR = ?, FILE_CT = ? WHERE ID_CT = ?";

        if($stmt = mysqli_prepare($db, $query)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssiissssssssssi", $param_NUM_CT, $param_DT_CT, $param_OBJ_CT, $param_ENTITE, 
            $param_ID_CTR, $param_ID_TYPE_CT, $param_PRM_CT,$param_AR1_CT, $param_AR2_CT, $param_MNT_BRUT, $param_MNT_BRUT_LTR, 
            $param_MNT_ARN, $param_MNT_ARN_LTR, $param_MNT_NET, $param_MNT_NET_LTR, $param_FILE_CT,$param_id);
    ...
    //mysqli_close($db);
        ...
        if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
            ...
            $query = "SELECT * FROM contrat WHERE ID_CT = ?";
            ...
            if($stmt = mysqli_prepare($db, $query)){
                ...
                if(mysqli_stmt_execute($stmt)){
                    ...
                    if(mysqli_num_rows($result) == 1){
                        ...
                        $AR1_CT = $row["AR1_CT"];
                        $AR2_CT = $row["AR2_CT"];
                        $MNT_BRUT = $row["MNT_BRUT"];
                        $MNT_BRUT_LTR = $row["MNT_BRUT_LTR"];
                        $MNT_ARN = $row["MNT_ARN"];
                        $MNT_ARN_LTR = $row["MNT_ARN_LTR"];
                        $MNT_NET = $row["MNT_NET"];
                        $MNT_NET_LTR = $row["MNT_NET_LTR"];
                        $FILE_CT = $row["FILE_CT"];
...
<!-- Card Welcome username -->
    ...
    <div class="form_centre1">
        <form class="formCTR" action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="POST" name="form1" id="form1">
            ...
            <fieldset class="border p-2">
                ...
                (!empty($ID_TYPE_CT_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $ID_TYPE_CT; ?>">
                    ...
                    echo '<option value="">اختر نوعية السابقة '.$ID_TYPE_CT.' او غيرها...</option>';
            ...
            <div class="form-group1">
                ...
                <?php
                    ...
                    echo '<option value="">اختر المتعاقد السابق '.$ID_CTR.' او غيره...</option>';
            ...
            <div class="form-group1">
                <div class="mb-3">
                    ...
                    <input class="form-control" name="FILE_CT" type="file" value="<?php echo $FILE_CT; ?>">
                </div>
            </div> 
